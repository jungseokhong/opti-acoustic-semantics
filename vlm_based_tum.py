...
DETECTOR__CONF_THRESH = 0.33  # 0.76  # Confidence threshold used for the detector, default is 0.5
OBJECT_DEPTH_TRHES = 3.0  # 3.0  # Depth threshold for objects, default is 5.0
DETECTOR_VISUALIZATION_THRESHOLD = 0.33 # visualization threshold for (RAM+DINO)/2
...
general_classes_to_remove = ['ceiling', 'floor', 'wall', 'room', 'classroom',
                             ...
                             'brown', 'sit', 'office building', 'multiple objects', 'pavement', 'pink',
                             'green', 'hand', 'cross'] # works only for SLAM landmarks, not for a detector
...
class ClosedSetDetector:
    ...
    def forward_pass(self, cam_info: CameraInfo, rgb: Image, depth: Image) -> None:
        ...
        if self.method == "yolo":
            ...
            depth_m = cv2.resize(depth_m, dsize=(640, 480), 
                                 interpolation=cv2.INTER_NEAREST)  # do this for realsense (img dim not a multiple of max stride length 32) 1280, 736

            # Run inference args: https://docs.ultralytics.com/modes/predict/#inference-arguments
            results = self.inference(image_cv, verbose=False, conf=CONF_THRESH, imgsz=(480, 640))[
        ...
        for mask, class_id, bboxes, dino_conf, ram_conf in zip(masks, class_ids, bboxes, confs, ram_confs):
            ...
            if ((conf < DETECTOR__CONF_THRESH) or (np.isnan(obj_depth)) or (np.isnan(obj_centroid[0]))
                    ...
                    or ((bboxes[2] - bboxes[0])*(bboxes[3] - bboxes[1])/ (640*480) >0.8)):
                if (bboxes[2] - bboxes[0])*(bboxes[3] - bboxes[1])/ (640*480) >0.7:
                    print("too big bbox", (bboxes[2] - bboxes[0])*(bboxes[3] - bboxes[1])/(640*480))
