...
from semanticslam_ros.srv import RemoveLandmark, RemoveLandmarkRequest, RemoveLandmarkResponse
...
class Compare2DMapAndImage:
    ...
    def forward_pass(self, yoloimg : RosImage, map_info : MapInfo, objects_info : ObjectsVector) -> None:
        ...
        position, orientation, landmark_points, landmark_classes, landmark_widths, landmark_heights, landmark_keys = self.parse_data(map_info)
        # Project landmarks to the image
        projected_image = self.projectLandmarksToImage(position, orientation, landmark_points, landmark_classes, landmark_widths, landmark_heights, landmark_keys, img = yoloimg_cv)
    ...
    def projectLandmarksToImage(self, position, orientation, landmark_points, landmark_classes, landmark_widths, landmark_heights, landmark_keys, img=None):
        ...
        for i, (point_3d, landmark_key, point_2d) in enumerate(zip(landmark_points, landmark_keys, points_2d_homo.T)):
            ...
            if 0 <= x < self.img_width and 0 <= y < self.img_height:
                ...
                self.landmark_keys.append(landmark_key)
